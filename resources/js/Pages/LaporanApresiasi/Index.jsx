...
export default function LaporanApresiasiIndex({ auth, laporanList, filters }) {
    ...
    function applyFilter(approval_status) {
        router.get(route('laporan-apresiasi.index'), {
    ...
    return (
        <GuruBkLayout user={auth.user} header="Laporan Apresiasi">
            <Head title="Laporan Apresiasi" />
            ...
            <div className="px-4 sm:px-6 lg:px-8 py-6 space-y-5 max-w-7xl mx-auto">
                    ...
                    <div className="flex items-start gap-3 bg-red-50 border border-red-300 rounded-xl px-4 py-3.5">
                        ...
                        <div className="flex-1">
                            <p className="text-sm font-semibold text-red-800">
                                {pendingBk} apresiasi memerlukan tindakan Anda
                            </p>
                            <p className="text-xs text-red-600 mt-0.5">
                                Sudah disetujui wali, menunggu diberikan reward oleh Guru BK.
                            </p>
                        </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div>
                        <h1 className="text-xl font-bold text-gray-900">Laporan Apresiasi</h1>
                                    ...
                                    {data.map(laporan => (
                                        <tr key={laporan.id}
                                            ...
                                            <td className="px-4 py-3">
                                                <span className="font-mono text-xs font-bold bg-emerald-100 text-emerald-700 px-2 py-1 rounded">
                                            ...
                                            <td className="px-4 py-3 max-w-[180px]">
                                                <p className="text-gray-700 truncate">{laporan.reward_default ?? '-'}</p>
                                            ...
                                            <td className="px-4 py-3">
                                                {laporan.bobot_poin != null
                                                    ? <span className="font-bold text-emerald-600">+{laporan.bobot_poin}p</span>
                                                    : <span className="text-gray-400">-</span>}
                                            </td>
                                            <td className="px-4 py-3">
                                            ...
                                            <td className="px-4 py-3">
                                                <Link href={route('laporan-apresiasi.show', laporan.id)}
                                                    className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition ${
                                                        laporan.approval_status === 'pending_bk'
                                                            ? 'bg-red-600 text-white hover:bg-red-700'
                                                            : 'bg-indigo-50 text-indigo-700 hover:bg-indigo-100'
                                                    }`}>
                                                    <Ic d={laporan.approval_status === 'pending_bk' ? PATH.star : PATH.eye} cls="w-3.5 h-3.5" />
                                                    {laporan.approval_status === 'pending_bk' ? 'Berikan' : 'Detail'}
                                                </Link>
                                            </td>
                                        </tr>
                                    ))}
