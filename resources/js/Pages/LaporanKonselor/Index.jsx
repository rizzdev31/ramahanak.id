/**
 * LaporanKonselor/Index.jsx
 * Sama pola dengan LaporanPelanggaran/Index.jsx
 */
...
const APPROVAL_CONFIG = {
    pending_tenaga_pendidik: { cls: 'bg-yellow-100 text-yellow-800 ring-yellow-300', dot: 'bg-yellow-500',              label: 'Menunggu Wali'  },
    pending_bk:              { cls: 'bg-red-100 text-red-800 ring-red-300',          dot: 'bg-red-500 animate-pulse', label: 'Perlu Aksi BK' },
    selesai:                 { cls: 'bg-emerald-100 text-emerald-800 ring-emerald-300', dot: 'bg-emerald-500',         label: 'Selesai'       },
    diabaikan:               { cls: 'bg-gray-100 text-gray-600 ring-gray-300',       dot: 'bg-gray-400',              label: 'Diabaikan'     },
};
...
export default function LaporanKonselorIndex({ auth, laporanList, filters }) {
    ...
    function applyFilter(approval_status) {
        router.get(route('laporan-konselor.index'), {
            approval_status, search: filters?.search ?? '',
    ...
    function applySearch(e) {
        e.preventDefault();
        router.get(route('laporan-konselor.index'), {
            approval_status: activeTab, search,
        }, { preserveState: true });
    }

    return (
        <GuruBkLayout user={auth.user} header="Laporan Konseling">
            <Head title="Laporan Konseling" />
            ...
            <div className="px-4 sm:px-6 lg:px-8 py-6 space-y-5 max-w-7xl mx-auto">
                    ...
                    <div className="flex items-start gap-3 bg-red-50 border border-red-300 rounded-xl px-4 py-3.5">
                        ...
                        <div className="flex-1">
                            <p className="text-sm font-semibold text-red-800">
                                {pendingBk} laporan konseling memerlukan tindakan Anda
                            </p>
                            <p className="text-xs text-red-600 mt-0.5">
                                Sudah disetujui wali, menunggu ditindaklanjuti oleh Guru BK.
                            </p>
                        </div>
                        <button type="button" onClick={() => applyFilter('pending_bk')}
                            className="shrink-0 text-xs font-semibold text-red-700 bg-red-100 hover:bg-red-200 px-3 py-1.5 rounded-lg transition">
                            Tampilkan
                        </button>
                    </div>
                )}
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div>
                        <h1 className="text-xl font-bold text-gray-900">Laporan Konseling</h1>
                    ...
                    <form onSubmit={applySearch} className="flex items-center gap-2">
                        <div className="relative">
                            ...
                            <input type="text" value={search} onChange={e => setSearch(e.target.value)}
                                ...
                                className="pl-9 pr-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 w-60" />
                        </div>
                        <button type="submit" className="px-3 py-2 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700">
                            Cari
                        </button>
                    </form>
                </div>
                ...
                <div className="flex gap-1 bg-gray-100 p-1 rounded-xl w-fit flex-wrap">
                    {TABS.map(tab => (
                        <button key={tab.key} type="button" onClick={() => applyFilter(tab.key)}
                            ...
                            {tab.key === 'pending_bk' && pendingBk > 0 && (
                                <span className="bg-red-500 text-white text-xs font-bold rounded-full min-w-[1.1rem] h-[1.1rem] flex items-center justify-center px-1">
                                    {pendingBk}
                                </span>
                            )}
                        </button>
                                    ...
                                    {data.map(laporan => (
                                        <tr key={laporan.id}
                                            className={`hover:bg-gray-50/60 transition-colors ${
                                                laporan.approval_status === 'pending_bk' ? 'bg-red-50/30 border-l-2 border-red-400' : ''
                                            }`}>
                                                ...
                                                {laporan.santri ? (
                                                    <div className="flex items-center gap-2">
                                                        <div className="w-7 h-7 rounded-full bg-blue-100 text-blue-700 font-bold text-xs flex items-center justify-center shrink-0">
                                                            {laporan.santri.nama?.[0]?.toUpperCase() ?? '?'}
                                                        </div>
                                                        <div>
                                                            <p className="font-medium text-gray-800">{laporan.santri.nama}</p>
                                                            <p className="text-xs text-gray-400">{laporan.santri.nisn}</p>
                                                        </div>
                                                    </div>
                                                ) : <span className="text-gray-400">-</span>}
                                            </td>
                                            <td className="px-4 py-3 max-w-[160px]">
                                                <p className="text-gray-700 truncate text-xs">{laporan.diagnosis_default ?? '-'}</p>
                                                {laporan.variabel?.gangguan_mental && (
                                                    <p className="text-xs text-gray-400 mt-0.5">{laporan.variabel.gangguan_mental}</p>
                                                )}
                                            </td>
                                            <td className="px-4 py-3 max-w-[160px]">
                                                <p className="text-gray-700 truncate text-xs">{laporan.tindakan_default ?? '-'}</p>
                                            </td>
                                            <td className="px-4 py-3">
                                                <ApprovalBadge status={laporan.approval_status} label={laporan.approval_status_label} />
                                                {laporan.has_overdue_approvals && (
                                                    <p className="text-xs text-red-500 mt-1 flex items-center gap-0.5">
                                                        <Ic d={PATH.clock} cls="w-3 h-3" /> Wali terlambat
                                                    </p>
                                                )}
                                            </td>
                                            <td className="px-4 py-3 w-32">
                                                <div className="text-xs text-gray-500">{laporan.approval_progress ?? 0}%</div>
                                            ...
                                            <td className="px-4 py-3">
                                                <Link href={route('laporan-konselor.show', laporan.id)}
                                                    className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition ${
                                                        laporan.approval_status === 'pending_bk'
                                                            ? 'bg-red-600 text-white hover:bg-red-700'
                                                            : 'bg-indigo-50 text-indigo-700 hover:bg-indigo-100'
                                                    }`}>
                                                    <Ic d={laporan.approval_status === 'pending_bk' ? PATH.heart : PATH.eye} cls="w-3.5 h-3.5" />
                                                    {laporan.approval_status === 'pending_bk' ? 'Tindak Lanjut' : 'Detail'}
                                                </Link>
                                            </td>
                                        </tr>
                                    ))}
                            ...
                            link.url ? (
                                <Link key={i} href={link.url}
                                    className={`px-3 py-1.5 text-sm rounded-lg transition ${
                                        link.active ? 'bg-indigo-600 text-white font-semibold' : 'bg-white border border-gray-200 text-gray-600 hover:bg-gray-50'
                                    }`}
                                    dangerouslySetInnerHTML={{ __html: link.label }} />
                            ) : (
                                <span key={i} className="px-3 py-1.5 text-sm rounded-lg bg-gray-50 text-gray-400"
                                    dangerouslySetInnerHTML={{ __html: link.label }} />
                            )
                        ))}
                    </div>
